...
export function OrdersPage() {
  ...
  useEffect(() => (user?.id ? subscribeUserCollection('orders', user.id, setOrders) : undefined), [user?.id])
  ...
  async function handleConfirmDelete() {
    ...
    setIsDeleting(true)
    try {
      if (deletingOrderTarget.isLocalDraft || targetId.startsWith('draft-')) {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 space-y-6">
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <p className="font-bold text-[#0b7e74] text-xs uppercase tracking-wider">{t('orders.customerDashboard')}</p>
          <h1 className="mt-1 text-2xl sm:text-3xl font-black">{t('orders.title')}</h1>
        ...
        <Link
          to="/custom-order"
          className="inline-flex items-center gap-2 rounded-lg bg-[#0b7e74]/15 px-4 py-2 text-xs font-bold text-[#0b7e74] transition hover:bg-[#0b7e74]/25 dark:bg-[#0b7e74]/25 dark:text-[#67dccf]"
        >
          <Sparkles className="h-3.5 w-3.5" />
          <span>{t('orders.viewCustomRequests')}</span>
      ...
      <div className="relative max-w-md">
        ...
        <input
          ...
          className="w-full rounded-lg border border-black/10 bg-white pl-10 pr-4 py-2 text-xs font-medium outline-none transition focus:border-[#0b7e74] dark:border-white/10 dark:bg-neutral-900"
        />
      </div>
      ...
      <div className="rounded-xl border border-black/10 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-neutral-900">
        {filteredOrders.length === 0 ? (
          <div className="p-12 text-center text-neutral-500">
            ...
            <p className="mt-2 text-sm font-bold">{t('orders.noOrdersTitle')}</p>
            <Link to="/store" className="mt-3 inline-block text-xs font-bold text-[#0b7e74] hover:underline">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-black/10 bg-neutral-50 font-bold uppercase tracking-wider text-neutral-500 dark:border-white/10 dark:bg-neutral-800/50">
                <tr>
                  <th className="p-3">{t('orders.orderRef')}</th>
                  <th className="p-3">{t('orders.itemsPurchased')}</th>
                  <th className="p-3">{t('orders.totalAmount')}</th>
                  <th className="p-3">{t('orders.paymentSource')}</th>
                  <th className="p-3">{t('orders.status')}</th>
                  <th className="p-3">{t('orders.orderDate')}</th>
                  <th className="p-3 text-right">{t('orders.action')}</th>
                </tr>
              </thead>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3 font-mono font-black text-[#0b7e74]">
                        <span className="bg-[#0b7e74]/10 px-2 py-0.5 rounded-md">
                          #{order.id.slice(0, 8)}
                        </span>
                      </td>
                      ...
                      {/* TOTAL AMOUNT */}
                      <td className="p-3 font-mono font-bold text-xs text-emerald-600 dark:text-emerald-400">
                      ...
                      <td className="p-3">
                        {order.paymentSource === 'wallet' ? (
                          <span className="inline-flex items-center gap-1 rounded-md bg-blue-500/10 px-2 py-0.5 text-[10px] font-semibold text-blue-600 dark:text-blue-400">
                            <CreditCard className="h-3 w-3" /> {t('orders.walletPay')}
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 rounded-md bg-amber-500/10 px-2 py-0.5 text-[10px] font-semibold text-amber-600 dark:text-amber-400">
                            <FileText className="h-3 w-3" /> {t('orders.manualTransfer')}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="p-3">
                        {order.status === 'delivered' ? (
                          <span className="inline-flex items-center gap-1 rounded-md bg-emerald-500/10 px-2 py-0.5 text-[10px] font-bold text-emerald-600 dark:text-emerald-400">
                            <CheckCircle2 className="h-3 w-3" /> {t('orders.delivered')}
                          </span>
                        ) : order.status === 'paid' ? (
                          ...
                          <span className="inline-flex items-center gap-1 rounded-md bg-amber-500/10 px-2 py-0.5 text-[10px] font-bold text-amber-600 dark:text-amber-400">
                            <Clock className="h-3 w-3" /> {t('orders.reviewing')}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="p-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <span className="inline-flex items-center gap-1 text-xs font-semibold text-[#0b7e74] hover:underline">
                            {t('orders.view')} <ChevronRight className="h-3.5 w-3.5" />
                          </span>
                          ...
                          {(isPending || order.isLocalDraft || order.id.startsWith('draft-')) && (
                            <button
                              ...
                              className="inline-flex items-center gap-1 rounded-md bg-rose-500/10 px-2 py-0.5 text-[11px] font-semibold text-rose-500 hover:bg-rose-500 hover:text-white transition cursor-pointer"
                              title={t('orders.cancel')}
                            >
                              <Trash2 className="h-3 w-3" /> {t('orders.cancel')}
                            </button>
                          )}
                        </div>
                      </td>
      ...
      <DeleteConfirmModal
        ...
        message={t('orders.cancelDeleteMsg', {
          id: deletingOrderTarget ? deletingOrderTarget.id.slice(0, 8) : '',
        })}
      />
    </section>
  )
